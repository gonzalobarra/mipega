...
(function( $ ) {
	...
	var easyWizardMethods = {
		init : function(options) {
			...
			return this.each(function() {
				...
				if(thisSettings.steps > 1) {
					...
					$stepsHtml = $('<ul class="easyWizardSteps nav nav-pills nav-justified">');
					...
					$steps.each(function(index) {
						...
						stepText = thisSettings.stepsText.replace('{t}', $(this).attr('data-step-title'));
						$htmlText = '<li'+(!index?' class="current active"':'')+' data-step="'+step+'"><a href="#"><h4 class="list-group-item-heading">Paso '+step+'</h4><p class="list-group-item-text">'+stepText+'</p></a></li>';
						$stepsHtml.append($htmlText);
					});
					...
					if(thisSettings.showButtons) {
						paginationHtml = '<div class="easyWizardButtons pull-right">';
							paginationHtml += '<button class="prev btn btn-warning'+thisSettings.buttonsClass+'"><span class="glyphicon glyphicon-chevron-left"></span> '+thisSettings.prevButton+'</button>';
							paginationHtml += '<button class="next btn btn-primary'+thisSettings.buttonsClass+'">'+thisSettings.nextButton+' <span class="glyphicon glyphicon-chevron-right"></span></button>';
		...
		goToStep : function(step) {
			...
			// Defines steps
			this.find('.easyWizardSteps .current').removeClass('current active');
			this.find('.easyWizardSteps li[data-step="'+step+'"]').addClass('current active');
